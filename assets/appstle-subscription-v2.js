  ...
  checkReady(function ($) {
    $(function () {
      ...
      function renderWidget(standAloneProduct, standAloneElement, widgetId) {
        ...
        var widgetLabels = RSConfig?.labels ? JSON.parse(RSConfig?.labels) : {};
              ...
              try {
                ...
                if (variant) {
                  ...
                  if (appstle_selling_plan_groups && appstle_selling_plan_groups.length > 0 && RS.Config.widgetEnabled) {
                    ...
                    if (MustacheWrapperJSON?.sellingPlanVariants?.length) {
                      var templateToRender = RSConfig?.widgetTemplateHtml?.replace('{% raw %}', '')?.replace('{% endraw %}', '')
                      var render = Mustache.render(templateToRender, MustacheWrapperJSON)
                      if (widgetPlacement === 'BEFORE') {
                        jQuery(render).insertBefore(atcButton);
                      } else if (widgetPlacement === 'AFTER') {
                        jQuery(render).insertAfter(atcButton);
                      } else if (widgetPlacement === 'FIRST_CHILD') {
                        jQuery(render).prependTo(atcButton);
                      } else if (widgetPlacement === 'LAST_CHILD') {
                        jQuery(render).appendTo(atcButton);
                    }
                  }
        ...
        function changeEventHandlerForSellingPlanWrapperRadio() {
          jQuery('#appstle_subscription_widget' + widgetId).find('input[type=radio][name=selling_plan_wrapper]').on('change',
            function (event) {
              var value = event.target.value;
              var currentSelectedSellingPlan = jQuery('[name="selling_plan"]:checked').val();
              $(this).prop("checked", true);
              if (value) {
                if (!currentSelectedSellingPlan) {
                  jQuery('[name="selling_plan"].appstleSellingPlanInput').first().prop("checked", true);
                  jQuery('[name="selling_plan"].appstleSellingPlanInput').first().change();
                }
              } else {
                if (currentSelectedSellingPlan) {
                  jQuery('[name="selling_plan"].appstleOneTimeInput').prop("checked", true);
                }
              }
              updateStateOfWidget();
            });
        ...
        function changeHandlerForSellingPlanInput() {
          jQuery(`#appstle_subscription_widget${widgetId} [name="selling_plan"]`).on('change', function (event) {
            if(event.target.value) {
              jQuery('#appstle_subscription_widget' + widgetId).find('input[type=radio][name=selling_plan_wrapper].appstle_sellingPlan_radio').prop("checked", true);
              jQuery(`#appstle_subscription_widget${widgetId} [name="selling_plan_child"]`).val(event.target.value);
              jQuery(`#appstle_subscription_widget${widgetId} [name="selling_plan_child"]`).change();
            } else {
              jQuery('#appstle_subscription_widget' + widgetId).find('input[type=radio][name=selling_plan_wrapper].appstle_onetime_radio').prop("checked", true);
            }
            updateStateOfWidget();
          });
        }

        function changeHandlerForSellingPlanChild() {
          jQuery(`#appstle_subscription_widget${widgetId} [name="selling_plan_child"]`).on('change', function (event) {
            if (event.target.value) {
              var sellingPlanRadio = jQuery('#appstle_subscription_widget' + widgetId).find('input[type=radio][name=selling_plan].appstleSellingPlanInput');
              sellingPlanRadio.prop("checked", true)
              sellingPlanRadio.val(event.target.value);
            } else {
              var oneTimeRadio = jQuery('#appstle_subscription_widget' + widgetId).find('input[type=radio][name=selling_plan].appstleSellingPlanInput');
              oneTimeRadio.prop("checked", true)
              oneTimeRadio.val(event.target.value);
            }
            updateStateOfWidget();
          })
        }
        ...
        function triggerSellingPlanWrapperChangeHandler() {
                if (RSConfig?.subscriptionOptionSelectedByDefault) {
                        jQuery('[name="selling_plan_wrapper"].appstle_sellingPlan_radio').change() //for Widget Type 1
                        jQuery('[name="selling_plan"].appstleSellingPlanInput').change() //for Widget Type 2
                } else {
                        jQuery('[name="selling_plan_wrapper"].appstle_onetime_radio').change() //for Widget Type 1
                        jQuery('[name="selling_plan"].appstleOneTimeInput').change() //for Widget Type 2
                }
        }
        ...
        function selectFirstSellingPlan() {
          if (RSConfig?.subscriptionOptionSelectedByDefault) {
            jQuery(`#appstle_subscription_widget${widgetId} .appstle_input_wrapper.appstle_subscription_option input[name="selling_plan"]`).first().attr('checked', true);
          }
          else{
            jQuery(`#appstle_subscription_widget${widgetId} .appstle_input_wrapper.appstle_onetime input[name="selling_plan"]`).first().attr('checked', true);
          }          
          triggerChangeEventForSellingPlan()
        }

        function triggerChangeEventForSellingPlan() {
          jQuery(`#appstle_subscription_widget${widgetId} [name="selling_plan"]`).change();
        }

        function registerAndTriggerEventsForFormFields() {
          changeEventHandlerForSellingPlanWrapperRadio();
          changeHandlerForSelect();
          changeHandlerForSellingPlanInput();
          changeHandlerForSellingPlanChild();
          triggerSellingPlanWrapperChangeHandler();
          selectFirstSellingPlan();
        ...
        if(!value){
          var NextSellingPlanElemnet = $("input[type='radio'].appstleSellingPlan" + widgetId)[1];
          value = $(NextSellingPlanElemnet).val();
        }
			} else {
        ...
        function updateOneTimePurchaseValueToRadio() {
          ...
          jQuery('#appstle_subscription_widget' + widgetId + ' .appstle_onetime_amount').html(wrapPriceWithSpanTag(buildOneTimePriceText(formatPriceWithQuantity(price))));
        }
        ...
        function updateSelectValueToRadio() {
          ...
          if (selectedSellingPlan) {
            ...
            jQuery('#appstle_subscription_widget' + widgetId + ' .appstle_selectedSellingPlan_amount').html(selectedPriceText);
            if (isPrepaidPlan && prepaidPerDeliveryPriceText) {
              jQuery('#appstle_subscription_widget' + widgetId + ' .appstle_selectedSellingPlan_amount_wrapper').append(
        ...
        function OneTimePriceWithQuantity(){
          var price = null;
          var selectorsObject = {
            regular: {
          ...
          var formatedPrice = buildOneTimePriceText(formatPriceWithQuantity(price));
          if(!checkIfSellingPlanGroupIsSelected() && sellingPrice.length === 1){
            jQuery(sellingPrice).html(formatedPrice);
          }          
        }
          ...
          var discountText = null;      
            $('.widgetSellingPlanWrapper .appstle_input_wrapper').each(function(){              
              if($(this).find('.appstle_lowercase.appstle_sellingPlan_price.appstle_onetime_amount').length === 0){                
                ...
                var selectedSellingPlanAllocation = getSellingPlanAllocation(variantId, parseInt(selectedSellingPlan));
                var isPrepaidPlan = selectedSellingPlanAllocation.per_delivery_price !== selectedSellingPlanAllocation.price
                var formattedSelectedSellingPlanPrice = formatPriceWithQuantity(isPrepaidPlan ? selectedSellingPlanAllocation.price : selectedSellingPlanAllocation.per_delivery_price); 
                $(this).find('span.appstle_lowercase.appstle_sellingPlan_price.appstle_price_block').html(formattedSelectedSellingPlanPrice);
              }
            }
            })
        ...
        function updateStateOfWidget() {
          ...
          OneTimePriceWithQuantity();
          FormatBoxPriceWithQuantity();
          var subsOption = jQuery('.appstle_subscribe_option');
          if (checkIfSellingPlanGroupIsSelected()) {
            subsOption.removeClass('appstle_hide_subsOption')
            jQuery('#appstle_subscription_widget' + widgetId + ' .appstle_subscription_wrapper_option.appstle_sellingplan_wrapper').addClass('appstle_selected_background');
            jQuery('#appstle_subscription_widget' + widgetId + ' .appstle_subscription_wrapper_option.appstle_onetime_wrapper').removeClass('appstle_selected_background')
          } else {
            jQuery('#appstle_subscription_widget' + widgetId + ' .appstle_subscription_wrapper_option.appstle_onetime_wrapper').addClass('appstle_selected_background')
    ...
    //           var sellingPlanGroup = jQuery('#appstle_subscription_widget' + widgetId + ' #appstle_selling_plan_label_2' + widgetId);
              ...
              // }
              let sellingPlanChildElement = formElement.find('[name=selling_plan_child]');
              if (sellingPlanChildElement.length) {
                sellingPlanChildElement.find(`option`).each(function (index, option) {
                  if (jQuery(option).attr('value') === paramsObj['selling_plan']) {
                    sellingPlanChildElement[0].selectedIndex = index;
                    sellingPlanChildElement.change();
                  }
                });
              } else {
                formElement.find('input[name=selling_plan]').each(function (index, option) {
